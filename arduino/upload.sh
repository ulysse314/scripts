...
/home/ulysse314/scripts/arduino/reset.py
if [ "$?" != 0 ]; then
  exit 2
fi
sleep 2
/home/ulysse314/arduino/arduino15/packages/arduino/tools/bossac/1.7.0/bossac -i -d --port=ttyACM0 -U true -i -e -w -v /tmp/arduino_build/arduino.ino.bin -R
if [ "$?" != 0 ]; then
  exit 3
fi
